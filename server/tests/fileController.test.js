...
describe('POST /api/files/contribution/abstract', () => {
  let team;
  let filePath;
  ...
  it('should upload a contribution abstract', async () => {
    const fileContent = 'Test file content';
    // Create a test picture file
    fs.writeFileSync(filePath, fileContent);

    const res = await request(app)
      .post('/api/files/contribution/abstract')
      .set('Authorization', `Bearer ${generateAccessToken(team._id)}`)
      .attach('file', filePath);
...
describe('POST /api/files/submission/abstract', () => {
  let team;
  let filePath;
  ...
  it('should upload a submission abstract', async () => {
    const fileContent = 'Test file content';
    // Create a test picture file
    fs.writeFileSync(filePath, fileContent);

    const res = await request(app)
      .post('/api/files/submission/abstract')
      .set('Authorization', `Bearer ${generateAccessToken(team._id)}`)
      .attach('file', filePath);
